...
了解如何成为认证 [!DNL Adobe Analytics] 开发人员专家。

## 最低体验

您应具有作为混合角色实施顾问的经验，并在以下领域拥有约2年的经验 [!DNL Adobe Analytics] 实现。 您充当IT和业务之间的桥梁，将业务需求转换为技术解决方案(SDR)，并根据分析师提供的需求，了解需要哪种类型的变量、在何处以及如何收集数据、设计数据模型（数据层）、配置和维护标签管理解决方案。
...
* 前端Web开发环境
* 了解移动应用程序（局限性和差异，例如，它与Web不同的原因）
* 数字营销领域
* 基本了解后端开发环境，包括PC、Mac、Android/iOS操作系统（基本操作，而非开发）
...
第2部分：基于解决方案设计参考(SDR)的战略和设计(6%)

* 解释解决方案设计参考以开发用于数据捕获的高效代码
* 使用技术规范填充数据对象

第3部分：Analytics实施和配置(36%)

* 确定要部署的环境 [!DNL Adobe Analytics] 代码
* 执行用于Analytics跟踪的最小部署代码
...
* 应用进程以通过Adobe启动配置网站标记
* 枚举启用扩展和Adobe启动的要求
* 创建规则和数据元素

第5节：移动服务(8%)

* 应用进程以配置和执行移动的基本预部署代码
* 演示对处理规则和移动服务的了解
...
* 推荐测试和调试最佳实践
* 疑难解答 [!DNL Adobe Analytics] 服务器调用在其生命周期内（即从信标请求到显示在报表中）
...
* [将移动服务处理规则迁移到 [!DNL Adobe Analytics]](https://experienceleague.adobe.com/docs/analytics/technotes/migrate-mobile.html){target="_blank"}
...
[!BADGE PSI]{type=Informative url="https://www.certmetrics.com/adobe/candidate/psi_sso_adobe.aspx?redir=yes&amp;ec=AD0-E209 newtab=true"}[!BADGE 考试]{type=Informative url="https://www.certmetrics.com/adobe/candidate/examity_sso.aspx?eid=AD0-E209 newtab=true"}
